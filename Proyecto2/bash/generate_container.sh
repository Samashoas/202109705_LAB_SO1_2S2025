...
for i in {1..10}; do
    ...
    if [ $imagen_seleccionada -eq 0 ]; then
        ...
        docker run -d --label "proyecto=so1_lab" --name "alto_consumo_ram_$i" $IMAGEN_ALTA_RAM bash -c "dd if=/dev/zero of=/tmp/testfile bs=1M count=100"
    elif [ $imagen_seleccionada -eq 1 ]; then
        echo "Creando contenedor CPU: alto_consumo_cpu_$i"
        docker run -d --label "proyecto=so1_lab" --name "alto_consumo_cpu_$i" $IMAGEN_ALTA_CPU bash -c "while true; do :; done"
    else
        echo "Creando contenedor básico: bajo_consumo_$i"
        docker run -d --label "proyecto=so1_lab" --name "bajo_consumo_$i" $IMAGEN_BAJA_RAM sleep 300
    fi
done
